...
public class GameRenderer implements GLSurfaceView.Renderer {
	...
	public GameRenderer(Context context, ImageDrawer imageDrawer, int modelResourceId, int textureResourceId) {
...
// Set the background clear color to black.
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		...
		meshRura1 = Mesh.getMeshSerialized(mModelResourceId, mResources, mTextureResourceId);
		//meshRura1 = Mesh.getMeshSerialized(mModelResourceId, mResources, mTextureResourceId);
	...
	public void onDrawFrame(GL10 gl) {
		...
		xrot = Math.max(Math.min(xrot, GameGLSurfaceView.MAX_X_ROT), GameGLSurfaceView.MIN_X_ROT);
...
//        } else {
		...
		//Matrix.scaleM(mModelMatrix, 0, 1, 1.5f, 1);
		...
		if(mModelRotationMatrix!=null) {
			...
			v[0] = mModelRotationMatrix[0];
			v[1] = mModelRotationMatrix[1];
			v[2] = mModelRotationMatrix[2];
			v[3] = mModelMatrix[3];

			v[4] = mModelRotationMatrix[3];
			v[5] = mModelRotationMatrix[4];
			v[6] = mModelRotationMatrix[5];
			v[7] = mModelMatrix[7];

			v[8] = mModelRotationMatrix[6];
			v[9] = mModelRotationMatrix[7];
			v[10] = mModelRotationMatrix[8];
			v[11] = mModelMatrix[11];

			v[12] = mModelMatrix[12];
			v[13] = mModelMatrix[13];
			v[14] = mModelMatrix[14];
			v[15] = mModelMatrix[15];
...
//        }
		...
		//Matrix.translateM(mModelMatrix, 0, 0.0f, 0.0f, -6);

		drawMesh(meshRura1, mProjectionMatrix);
	}
	...
	public void drawMesh(Mesh mesh, float[] mProjectionMatrix) {
		...
		GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, mesh.getTextureDataHandle(mImageDrawer));
